...
| Sinyal putus (tidak ada respons HTTP) | **Tidak dihitung gagal & job tidak pernah dibuang.** Jeda 2 → 30 dtk + jitter; saat `navigator.onLine === false` tidak mencoba sama sekali dan menunggu event `online`. Tamu melihat "offline · N tersimpan di HP" |
| Server error (ada respons HTTP) | Dihitung di `attempts` hanya untuk memperlambat jeda (2 dtk → 5 menit); job tetap tidak dibuang |
| Putus saat upload versi film / thumbnail | Seluruh job diulang, tidak di-commit tanpa versi itu. Kalau yang menolak server (bukan jaringan), foto asli tetap di-commit tanpa versi bonus |
| Dua upload paralel tabrakan rate limit | `TERLALU_CEPAT` dicoba ulang 700 ms tanpa dihitung gagal |
| Film habis / sesi mati / acara tutup | Job dibuang — **satu-satunya** kondisi foto keluar dari antrean tanpa terkirim |
...
| Upload paralel / jeda jaringan / jeda server | 2 / 2–30 dtk / 2 dtk–5 menit | `lib/upload/uploader.ts` |
...
|---|---|---|
| Job antrean dibuang bila sesi kedaluwarsa (18 jam) atau acara sudah ditutup | Foto tamu yang baru mendapat sinyal setelah kamera ditutup hilang | Tutup kamera 30–60 menit setelah acara selesai ([OPERASIONAL.md](OPERASIONAL.md)) |
...
| Antrean hanya berjalan selama halaman kamera / selesai terbuka | Tamu yang menutup tab saat offline baru mengirim foto ketika membuka link acara lagi | Peringatan `beforeunload` saat masih ada antrean; foto tetap aman di IndexedDB |
